...
export default function InitiativeDetailPage({
  ...
  async function handleConfirmDelete() {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`/api/initiatives/${id}`, { method: 'DELETE', cache: 'no-store' });
      let raw = '';
      try {
        raw = await res.clone().text();
      } catch {}
      if (!res.ok && res.status !== 204) {
        let msg = 'Failed to delete initiative';
        try {
      ...
      showToast({ title: 'Initiative deleted', description: 'The initiative was deleted successfully.' });
        ...
        {isEditing ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="mt-1 text-gray-900">{initiative.TeamMember?.name || 'Unassigned'}</p>
            </div>
            <div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="mt-2 text-sm text-gray-600">Are you sure you want to delete this initiative? This action cannot be undone.</p>
