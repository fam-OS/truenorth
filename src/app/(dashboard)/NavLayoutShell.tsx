...
export default function NavLayoutShell({ children }: { children: ReactNode }) {
  ...
  }, [open, focusedIndex, apps, userMenuOpen, router]);
          ...
          <div className="flex justify-between h-28">
            ...
            <div className="flex items-center gap-3">
              ...
              {status === 'authenticated' && session?.user ? (
                  ...
                  <div ref={userMenuRef} className="relative">
                    ...
                    {userMenuOpen && (
                      <div className="absolute right-0 top-10 z-50 w-48 rounded-md border bg-white shadow-lg py-1">
                        <Link href="/settings" onClick={() => setUserMenuOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                          <CogIcon className="h-4 w-4 mr-3 text-gray-500" />
                          Settings
                        </Link>
                        <Link href="/feature-request" onClick={() => setUserMenuOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                          <ClipboardDocumentListIcon className="h-4 w-4 mr-3 text-gray-500" />
                          Feature Request
                        </Link>
                        <Link href="/support" onClick={() => setUserMenuOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                ...
                <Link href="/api/auth/signin" className="inline-flex items-center rounded-md bg-blue-600 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                  Sign in
                </Link>
              ) : null}
            </div>
