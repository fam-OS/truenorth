...
export default function DeleteOpsReviewItemButton({ reviewId, itemId }: { reviewId: string; itemId: string }) {
  ...
  const handleDelete = async () => {
    try {
      setDeleting(true);
      const res = await fetch(`/api/ops-reviews/${reviewId}/items/${itemId}`, { method: "DELETE" });
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(body?.error || `Failed to delete (status ${res.status})`);
      }
      showToast({ title: "Item deleted", description: "The ops review item was removed." });
      router.push(`/ops-reviews/${reviewId}`);
      router.refresh();
    } catch (e) {
